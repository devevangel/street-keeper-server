...
| Code | HTTP | Description | When It Occurs |
|------|------|-------------|----------------|
| `ROUTE_NOT_FOUND` | 404 | Route does not exist | Invalid route ID or route was deleted |
| `ROUTE_INVALID_RADIUS` | 400 | Invalid radius value | Radius not in [500, 1000, 2000, 5000, 10000] |
| `ROUTE_NO_STREETS` | 400 | No streets found in area | Selected area has no mapped streets in OSM |
| `ROUTE_ACCESS_DENIED` | 403 | Not authorized to access route | Trying to access another user's route |
...
|-------------|-------------|---------|
| **400 Bad Request** | `AUTH_DENIED`, `AUTH_MISSING_CODE`, `ROUTE_INVALID_RADIUS`, `ROUTE_NO_STREETS`, `GPX_*`, `VALIDATION_ERROR`, `WEBHOOK_INVALID_SIGNATURE` | Client error - fix the request |
| **401 Unauthorized** | `AUTH_INVALID_CODE`, `AUTH_TOKEN_EXPIRED`, `AUTH_REQUIRED` | Authentication required or failed |
| **403 Forbidden** | `ROUTE_ACCESS_DENIED`, `WEBHOOK_VERIFICATION_FAILED` | Authenticated but not authorized |
| **404 Not Found** | `ROUTE_NOT_FOUND`, `ACTIVITY_NOT_FOUND`, `NOT_FOUND` | Resource doesn't exist |
    ...
    if (error instanceof ApiError) {
      switch (error.code) {
        ...
        case 'ROUTE_NOT_FOUND':
          // Show "route not found" message
          showError('This route no longer exists.');
          break;
        
        case 'OVERPASS_API_ERROR':
          // External service down - suggest retry
          showError('Street data service is temporarily unavailable. Please try again in a few minutes.');
          break;
...
function getErrorCategory(code: string): string {
  ...
  if (code.startsWith('ROUTE_')) return 'route';
...
const errorMessages: Record<string, string> = {
  ...
  'ROUTE_INVALID_RADIUS': 'Please select a valid radius.',
