...
| GET | `/celebrations/history` | Query: optional **`cursor`** (`<iso>\|<activityId>` from previous response), **`limit`** (default 20, max 50), **`projectId`** (scope to one project). Returns `{ success, entries, nextCursor }`. `entries` are activity-grouped celebrations ordered newest first; each entry carries rollup counts, `acknowledged`, `sharedToStrava`, and the full list of per-project `events` (same shape as `/pending`). Powers the Run Journal page and per-project celebration list. |
