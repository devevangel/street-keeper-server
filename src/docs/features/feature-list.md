...
- **V2 (node proximity):** CityStrides-style 25 m node snap → node hits in `UserNodeHit`; completion derived at query time (90% rule)

All features below build on existing data: `UserNodeHit` (V2), `UserStreetProgress` (V1), `Activity`, `User`, `Project`, and Strava webhook sync. Features are engine-agnostic where possible — they read from whichever progress table is active.
